...
TEST_CASE("poly sprintf_l tests", "[printf]") {
    ...
    SECTION("Integer numbers") {
        ...
        CHECK(ret == expected.size()); REQUIRE(expected == result);
        ...
        CHECK(ret == expected.size()); REQUIRE(expected == result); // FAIL
...
TEST_CASE("poly snprintf_l tests", "[printf]") {
    ...
    CHECK(snprintf_l(buf, 100, " %s     %d", ploc, "b", 123) == 10);
    result = buf;
    REQUIRE(result == " b     123");
